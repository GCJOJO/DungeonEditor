...
#include "WriteAchievementProxy.generated.h"

/**
 * 
 */
DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FWriteAchievementDelegate, FName, WrittenAchievementName, float, WrittenProgress, int32, WrittenUserTag);

UCLASS(MinimalAPI)
class UWriteAchievementProxy : public UOnlineBlueprintCallProxyBase
{
	...
	UPROPERTY(BlueprintAssignable)
	FWriteAchievementDelegate OnFailure;
	...
	static UWriteAchievementProxy* WriteAchievementProgress(UObject* WorldContextObject, int32 localPlayerID, FName AchievementName, float Progress = 100.0f, int32 UserTag = 0);
